...
# ad-hoc SQL against the service - sqlcmd ships in the mssql image, not this one,
# so shell out to that container (the docker CLI is available here via docker-outside-of-docker)
docker exec sqlitemanager-dev-mssql-1 /opt/mssql-tools18/bin/sqlcmd \
